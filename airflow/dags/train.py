...
def main():
    ...
    feature_columns = ["Order_Year", "Order_Month", "Units Sold", "Unit Price", "Unit Cost"] + \
    ...
    gbt = GBTRegressor(featuresCol="scaled_features", labelCol="Total Revenue", predictionCol="Predicted_Revenue")

    pipeline = Pipeline(stages=indexers + encoders + [assembler, scaler, gbt])
    model = pipeline.fit(train_data)

    predictions = model.transform(test_data)
    evaluator = RegressionEvaluator(labelCol="Total Revenue", predictionCol="Predicted_Revenue", metricName="rmse")
    rmse = evaluator.evaluate(predictions)
    print(rmse)

    model.save("/tmp/revenue_model")
